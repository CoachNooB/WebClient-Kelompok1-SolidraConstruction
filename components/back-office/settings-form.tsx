...
import type { CompanySettings } from "@/lib/repositories/public-content";
export function SettingsForm({ value }: { value: CompanySettings }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const body = {
      ...
      descriptionId: form.get("descriptionId"),
    ...
    const response = await fetch("/api/back-office/settings", {
      method: "PUT",
    ...
    setMessage(
      response.ok
        ? { text: "Settings saved", tone: "success" }
        : { text: "Unable to save settings", tone: "error" },
    );
  }
  const social = value.socialLinks ?? {};
  const seo = value.defaultSeo ?? {
    title: value.name,
    description: "Solidra Construction corporate website",
  };
  return (
    <form onSubmit={submit} className="card mt-8 grid gap-4">
      <h2 className="text-xl font-bold">Company identity</h2>
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <label>
          ...
          <select
            name="defaultLocale"
            defaultValue={value.defaultLocale}
      ...
      <label>
        ...
        <textarea
          name="descriptionId"
          defaultValue={value.descriptionId}
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Input
          ...
          placeholder="LinkedIn HTTPS URL"
        />
        <Input
          name="instagram"
          defaultValue={social.instagram}
          placeholder="Instagram HTTPS URL"
        />
        <Input
          name="youtube"
