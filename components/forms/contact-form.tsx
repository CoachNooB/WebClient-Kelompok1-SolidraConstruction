  ...
  return (
    <form onSubmit={submit} className="card grid gap-5">
      ...
      {state === "success" && (
        <div
          role="status"
          className="rounded border border-green-300 bg-green-50 p-3 text-green-900"
        >
          {locale === "id" ? "Pesan telah diterima." : "Message received."}
        </div>
      )}
      ...
      {state === "error" && (
        <p role="alert" className="text-red-600">
          {locale === "id" ? "Pesan tidak dapat dikirim." : "Unable to submit."}
        </p>
      )}
