...
export async function publishRevision(
  ...
  await prisma.$transaction(async (transaction) => {
    await transaction.pageRevision.update({
      where: { id: revisionId },
      data: { immutable: true, publisherId, publishedAt: now },
    });
    ...
    await transaction.auditLog.create({
      data: {
        ...
        entity: "PageRevision",
        entityId: revisionId,
        metadata: { pageId: revision.pageId },
      },
    });
  });
  await invalidate([
    cacheKeys.page("ID", revision.page.key),
    cacheKeys.page("EN", revision.page.key),
    cacheKeys.navigation("ID"),
    cacheKeys.navigation("EN"),
    cacheKeys.footer("ID"),
    cacheKeys.footer("EN"),
  ]);
