...
export const vacancyEditorSchema = z.object({
  department: z.string().trim().min(2).max(100),
  location: z.string().trim().min(2).max(100),
  employmentType: z.enum(["FULL_TIME", "PART_TIME", "CONTRACT", "INTERNSHIP"]),
  closingDate: z.string().refine((value) => !Number.isNaN(Date.parse(value)), "Invalid closing date"),
  translations: z.array(translationSchema).length(2),
}).refine((data) => {
  const locales = data.translations.map((translation) => translation.locale);
  return locales.includes("ID") && locales.includes("EN") && new Set(locales).size === 2;
}, { path: ["translations"], message: "Exactly one ID and one EN translation is required" });
