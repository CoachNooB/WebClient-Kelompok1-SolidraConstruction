...
describeIntegration("cms workflows", () => {
  it("saves draft revisions and publishes them with audit logs", async () => {
      ...
      translations: [
        {
          locale: "ID",
          title: "Halaman Uji",
          description: "Deskripsi halaman uji",
        },
        {
          locale: "EN",
          title: "Test Page",
      ...
      sections: [
        {
          ...
          mediaId: null,
          translations: [
            { locale: "ID", heading: "Konten", body: "Isi konten" },
            { locale: "EN", heading: "Content", body: "Content body" },
          ],
        },
      ],
    });
    ...
    ).resolves.toEqual({ publishedRevisionId: originalPublishedRevisionId });
    ...
    ).resolves.toEqual({ publishedRevisionId: draft.revisionId });
    await expect(
      prisma.auditLog.findFirst({
        where: { action: "PAGE_PUBLISHED", entityId: draft.revisionId },
      }),
    ).resolves.toBeTruthy();
  });
});
