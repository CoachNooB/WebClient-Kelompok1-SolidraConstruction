...
describeIntegration("database schema", () => {
  it("keeps application tables in solidra schema", async () => {
    ...
    >`select schema_name from information_schema.schemata where schema_name = 'solidra'`;
    ...
    const tables = await prisma.$queryRaw<
      Array<{ table_name: string }>
    >`select table_name from information_schema.tables where table_schema = 'solidra' and table_type = 'BASE TABLE'`;
    ...
    [
      ...
      "career_applications",
      "audit_logs",
    ].forEach((name) => expect(tableNames.has(name)).toBe(true));
  });

  it("uses snake_case columns for mapped fields", async () => {
    const columns = await prisma.$queryRaw<
      Array<{ table_name: string; column_name: string }>
    >`select table_name, column_name from information_schema.columns where table_schema = 'solidra'`;
    const names = new Set(
