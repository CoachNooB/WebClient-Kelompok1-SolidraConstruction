...
import { getCompanySettings, getNavigation, getPublishedPage } from "@/lib/repositories/public-content";
...
describeIntegration("public repositories", () => {
  it("returns settings, fallback page, and navigation data", async () => {
    await expect(getCompanySettings()).resolves.toEqual(expect.objectContaining({ name: expect.any(String) }));
