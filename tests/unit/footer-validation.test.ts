...
describe("footerEditorSchema", () => {
  ...
  it("accepts one valid external HTTPS link", () => {
    expect(
      footerEditorSchema.safeParse({
        ...base,
        links: [{ ...link, external: true, url: "https://example.com" }],
      }).success,
    ).toBe(true);
  });
  ...
  it("rejects unsafe external URL", () => {
    expect(
      footerEditorSchema.safeParse({
        ...base,
        links: [{ ...link, external: true, url: "http://example.com" }],
      }).success,
    ).toBe(false);
  });
