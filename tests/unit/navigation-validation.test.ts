...
describe("navigationEditorSchema", () => {
  it("accepts internal URL", () => {
    expect(
      navigationEditorSchema.safeParse({ ...base, url: "/about" }).success,
    ).toBe(true);
  });

  it("rejects protocol-relative internal URL", () => {
    expect(
      navigationEditorSchema.safeParse({ ...base, url: "//evil.example" })
        .success,
    ).toBe(false);
  });
  ...
  it("accepts external HTTPS URL", () => {
    expect(
      navigationEditorSchema.safeParse({
        ...base,
        external: true,
  ...
  it("normalizes empty parentId to null", () => {
    expect(
      navigationEditorSchema.parse({ ...base, parentId: "" }).parentId,
    ).toBeNull();
  });
});
