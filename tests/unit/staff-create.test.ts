...
describe("staff account creation errors", () => {
  it("reports duplicate staff emails clearly", () => {
    ...
    expect(createStaffAccountErrorResponse(error)).toEqual({
  ...
  it("uses a generic conflict for other creation failures", () => {
    expect(
      createStaffAccountErrorResponse(new Error("database unavailable")),
    ).toEqual({ error: "Unable to create staff account", status: 409 });
  });
});
